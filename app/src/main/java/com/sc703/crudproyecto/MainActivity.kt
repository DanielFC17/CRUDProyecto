...
class MainActivity : AppCompatActivity() {
    ...
    private fun addStudent(){
        ...
        if(name.isEmpty() || email.isEmpty()){
            Toast.makeText(this, "Ingrese el campo requerido", Toast.LENGTH_SHORT).show()
        }else{
    ...
    private fun updateStudent(){
        ...
        if(name == std?.name && email == std?.email){
            Toast.makeText(this, "Cambios no guardados...", Toast.LENGTH_SHORT).show()
            return
        }
        if(std == null) return
        ...
        if(status > -1) {
            ...
            Toast.makeText(this, "Modificación fallada", Toast.LENGTH_SHORT).show()
        }
    ...
    private fun deleteStudent(id:Int){
        ...
        builder.setMessage("¿Seguro que quiere eliminar este registro?")
